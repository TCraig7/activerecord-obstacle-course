...
describe 'ActiveRecord Obstacle Course' do
  ...
  it 'finds orders by amount' do
    ...
    # The question mark is a binding parameter so people can't mess with your stuff
    # ------------------------------------------------------------
  ...
  it 'finds order id of smallest order' do
    # ----------------------- Using Raw SQL ----------------------
    # order_id = ActiveRecord::Base.connection.execute('SELECT id FROM orders ORDER BY amount ASC LIMIT 1')
    # ------------------------------------------------------------
  ...
  it 'should return all items except items: 3, 4 & 5' do
    items_not_included = [item_3, item_4, item_5, item_6]
  ...
  it 'returns all orders for user 2 which include item_4' do
    ...
    .where('order_items.item_id = ? AND orders.user_id = ?', item_4.id, user_2.id)
    # -----------------------------------------------------------
  ...
  it 'returns a table of information for all users orders and item counts' do
    ...
    # .select("count(order_items.id) as item_count")
